<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="仓储费" prop="warehousing" align="center" />
      <el-table-column label="搬运费" prop="truckage" align="center" />
      <el-table-column label="仓库类型" prop="type" align="center" />
      <el-table-column label="备注" prop="remark" align="center" />
      <el-table-column label="默认仓库" align="center" width="100">
    ...
    <!-- 添加或修改仓库对话框 -->
    <el-dialog
      :title="dialogStatus === 'create' ? '添加仓库' : '编辑仓库'"
    ...
    >
      ...
      >
        ...
        <el-form-item label="仓库地址" prop="address">
          <el-input v-model="temp.address" placeholder="请输入仓库地址" />
        </el-form-item>
        <el-form-item label="仓储费" prop="warehousing">
          <el-input v-model="temp.warehousing" placeholder="请输入仓储费" />
        </el-form-item>
        <el-form-item label="搬运费" prop="truckage">
          <el-input v-model="temp.truckage" placeholder="请输入搬运费" />
        </el-form-item>
        <el-form-item label="仓库类型" prop="type">
          <el-input v-model="temp.type" placeholder="请输入仓库类型" />
        </el-form-item>
...
import { listDepots, addDepot, updateDepot, deleteDepot, setDefaultDepot } from '@/api/depot'
...
const getList = async () => {
  ...
  try {
    const response = await listDepots(listQuery)
...
const createData = async () => {
  try {
    ...
    const response = await addDepot(temp)
...
const updateData = async () => {
  try {
    await dataFormRef.value.validate()
    const response = await updateDepot(temp)
    if (response.code === 200 && response.data) {
      dialogFormVisible.value = false
      ElMessage({
        message: '更新成功',
...
const handleSetDefault = (row) => {
  ElMessageBox.confirm('确认将该仓库设为默认仓库吗?', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    try {
      const response = await setDefaultDepot(row.id)
      if (response.code === 200 && response.data) {
        ElMessage({
          message: '设置成功',
          type: 'success'
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确认删除该仓库吗?', '警告', {
    ...
    try {
      const response = await deleteDepot(row.id)
      if (response.code === 200 && response.data) {
        ElMessage({
          message: '删除成功',
          type: 'success'

<template>
  <div class="app-container">
    ...
    <!-- 添加/编辑销售对话框 -->
    <el-dialog
      :title="dialogStatus === 'create' ? '添加销售记录' : '编辑销售记录'"
...
const getList = async () => {
  ...
  try {
    const response = await getSalesPage(listQuery)
...
const getCustomerOptions = async () => {
  try {
    const response = await listCustomers({ current: 1, size: 100 })
...
const handleDetail = async (row) => {
  try {
    ...
    if (response.code === 200 && response.data) {
      Object.assign(currentSales, response.data)
